...
def generate_wg_config_string(server_config, clients_data, server_interface_name="wg0"):
    ...
    server_interface_ip_cidr = None

    if isinstance(server_address_config, str) and server_address_config.strip():
        addr_strip = server_address_config.strip()
        if '/' in addr_strip: # Formato "10.10.10.1/24"
            parts = addr_strip.split('/')
            ip_parts_check = parts[0].split('.')
            if len(ip_parts_check) == 4 and \
               all(p.isdigit() and 0 <= int(p) <= 255 for p in ip_parts_check) and \
               parts[1].isdigit() and 0 <= int(parts[1]) <= 32:
                server_interface_ip_cidr = addr_strip
            else:
                console.print(f"[bold red]Error:[/bold red] El 'address' del servidor ('{addr_strip}') en '{WG_CONFIG_FILE}' no es un formato IP/CIDR válido.")
        elif addr_strip.endswith('.'): # Formato "10.10.10." (prefijo)
            prefix_parts_check = addr_strip[:-1].split('.')
            if len(prefix_parts_check) == 3 and \
               all(p.isdigit() and 0 <= int(p) <= 255 for p in prefix_parts_check):
                # Asumir .1 para el servidor y /24 para la subred
                server_interface_ip_cidr = f"{addr_strip}1/24"
                console.print(f"[info]El 'address' del servidor en '{WG_CONFIG_FILE}' es un prefijo ('{addr_strip}').[/info]")
                console.print(f"[info]Se usará '{server_interface_ip_cidr}' para la interfaz WireGuard del servidor.[/info]")
            else:
                console.print(f"[bold red]Error:[/bold red] El prefijo 'address' del servidor ('{addr_strip}') en '{WG_CONFIG_FILE}' no es un formato xxx.yyy.zzz. válido.")
        else:
            console.print(f"[bold red]Error:[/bold red] El 'address' del servidor ('{addr_strip}') en '{WG_CONFIG_FILE}' no tiene un formato reconocido (ej: '10.10.10.1/24' o '10.10.10.').")
    
    if server_interface_ip_cidr:
        config_lines.append(f"Address = {server_interface_ip_cidr}")
    else:
        console.print(f"[bold red]Error Crítico:[/bold red] No se pudo determinar la IP de la interfaz WireGuard del servidor desde '{WG_CONFIG_FILE}'.")
